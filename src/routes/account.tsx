...
function AccountPage() {
  ...
  useEffect(() => {
    ...
    // Check if any admin exists to allow first-time setup
    if (!isAdmin) {
  ...
  const claimAdmin = async () => {
    const { error } = await supabase.rpc("promote_first_admin");
    if (error) return toast.error(error.message);
    toast.success("تم منحك صلاحيات المسؤول بنجاح!");
    window.location.reload(); // Reload to update isAdmin state
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background" dir="rtl">
      <Header />
      <main className="flex-1 pt-[100px] md:pt-[120px] pb-8 px-4">
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-primary/15 via-card to-secondary/10 rounded-3xl p-6 md:p-8 mb-6 border border-border shadow-sm animate-fade-in">
            ...
            <div className="relative flex flex-col md:flex-row md:items-center gap-6">
              ...
              <div className="flex gap-2">
                {noAdmins && !isAdmin && (
                  <Button variant="secondary" size="sm" onClick={claimAdmin} className="bg-amber-500 hover:bg-amber-600 text-white">
          ...
          <Tabs defaultValue="orders">
            <TabsList className="grid grid-cols-4 mb-6">
              <TabsTrigger value="orders">
                <ShoppingBag className="w-4 h-4 ml-1" /> طلباتي
              </TabsTrigger>
              <TabsTrigger value="wishlist">
                <Heart className="w-4 h-4 ml-1" /> المفضلة
              </TabsTrigger>
              <TabsTrigger value="addresses">
                <MapPin className="w-4 h-4 ml-1" /> العناوين
              </TabsTrigger>
              <TabsTrigger value="notifications">
                <Bell className="w-4 h-4 ml-1" /> الإشعارات
              </TabsTrigger>
            </TabsList>
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2 flex-wrap">
        {[
          { k: "all", l: "الكل" },
          ...STATUS_STEPS.map((s) => ({ k: s.key, l: s.label })),
          { k: "cancelled", l: "ملغى" },
        ].map((f) => (
          <button
            key={f.k}
        ...
        filtered.map((o) => {
          ...
          return (
            <div key={o.id} className="bg-card border border-border rounded-xl p-5 transition-all">
                  ...
                  {STATUS_STEPS.map((s, i) => (
                    <div key={s.key} className="flex-1 flex items-center">
                      ...
                      {i < STATUS_STEPS.length - 1 && (
                        <div
                          className={`flex-1 h-1 mx-1 rounded ${i < stepIdx ? "bg-primary" : "bg-muted"}`}
                        />
                      )}
                    </div>
                  ))}
              ...
              <button
                onClick={() => setOpened(isOpen ? null : o.id)}
              ...
              {isOpen && (
                <div className="mt-4 pt-4 border-t border-border space-y-3 animate-fade-in">
                  <div className="space-y-2">
                    {(Array.isArray(o.items) ? (o.items as unknown as CartItem[]) : []).map(
                      (it, i) => (
                        <div key={i} className="flex items-center gap-3 text-sm">
                          ...
                          <span className="text-muted-foreground">
                            {it.quantity}× {Number(it.price).toFixed(2)} د.ت
                          </span>
                        </div>
                      ),
                    )}
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    >
                      <Button variant="outline" size="sm">
                        <MessageCircle className="w-4 h-4 ml-1" /> تواصل
                      </Button>
                    </a>
                    {o.status === "pending" && (
...
function WishlistTab() {
  ...
  const [items, setItems] = useState<
    (Tables<"wishlists"> & { products: Tables<"products"> | null })[]
  >([]);
  ...
  const load = useCallback(async () => {
    ...
    supabase
      .from("wishlists")
      .select("id,product_id,created_at,products:product_id(*)")
      .eq("user_id", user.id)
      .then(({ data }) =>
        setItems(
          (data as unknown as (Tables<"wishlists"> & { products: Tables<"products"> | null })[]) ||
            [],
        ),
      );
  }, [user]);
      ...
      {items.map((it) => {
        ...
        return (
          <div key={it.id} className="bg-card rounded-xl border border-border overflow-hidden">
            <img src={p.image ?? undefined} alt={p.name} className="w-full h-40 object-cover" />
            <div className="p-3">
  ...
  return (
    <div className="space-y-4">
      ...
      {adding && (
        <form
          onSubmit={save}
          className="bg-card border border-border rounded-xl p-5 grid sm:grid-cols-2 gap-3"
        >
          <Input
            ...
            >
              <div className="flex justify-between items-start gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {new Date(n.created_at).toLocaleDateString("ar-TN")}
                </span>
              </div>
            </div>
          ))}
