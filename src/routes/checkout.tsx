...
export const Route = createFileRoute("/checkout")({
  head: () => ({ meta: [{ title: "إتمام الطلب — Kusuf" }] }),
...
function CheckoutPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.rpc("place_order", {
      _items: items.map((i) => ({ id: i.id, quantity: i.quantity })) as unknown as Json,
      _shipping: form as unknown as Json,
      _phone: form.phone,
  ...
  if (done) {
    return (
      <div dir="rtl" className="min-h-screen flex flex-col">
        <Header />
        <main className="flex-1 flex items-center justify-center px-4 pt-[100px] md:pt-[120px]">
  ...
  return (
    <div dir="rtl" className="min-h-screen flex flex-col">
      <Header />
      <main className="flex-1 pt-[100px] md:pt-[120px] pb-12">
          ...
          <div className="bg-card p-6 rounded-2xl border border-border space-y-4 h-fit">
            ...
            <div className="space-y-2 pt-3 border-t border-border text-sm">
              ...
              <div className="flex justify-between font-bold text-lg pt-2 border-t border-border">
                <span>الإجمالي</span>
                <span className="text-primary">{(total + shipping).toFixed(2)} د.ت</span>
              </div>
            </div>
          </div>
        </div>
